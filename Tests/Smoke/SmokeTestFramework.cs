...
namespace WhisperKey.Tests.Smoke
{
    ...
    public class SmokeTestEnvironmentManager
    {
        ...
        public async Task<bool> SetupProductionEnvironmentAsync()
        {
            try
            {
                _logger.Information("Setting up smoke test environment for production validation");

                // Validate environment configuration
                if (!await ValidateEnvironmentConfigurationAsync())
                {
                    _logger.Error("Environment configuration validation failed");
                    return false;
                }
                ...
                _logger.Information("Smoke test environment setup completed successfully");
                return true;
            }
            catch (Exception ex)
            {
                _logger.Error(ex, "Failed to setup smoke test environment");
                return false;
            }
        }
        ...
        public async Task CleanupProductionEnvironmentAsync()
        {
            try
            {
                _logger.Information("Cleaning up smoke test environment");
                ...
                foreach (var disposable in _disposables)
                {
                    ...
                    {
                        _logger.Warning(ex, "Error disposing resource during cleanup");
                    }
                }
                _disposables.Clear();

                await Task.Delay(1000); // Allow for cleanup completion
                _logger.Information("Smoke test environment cleanup completed");
            }
            catch (Exception ex)
            {
                _logger.Error(ex, "Error during environment cleanup");
            }
        }
        ...
        private async Task<bool> ValidateEnvironmentConfigurationAsync()
        {
            ...
            foreach (var varName in requiredVars)
            {
                ...
                {
                    _logger.Error("Required environment variable {VarName} is not set", varName);
                    return false;
                }
            }
            ...
            {
                _logger.Error("Network connectivity validation failed");
                return false;
            }
    ...
    public class SmokeTestResultCollector
    {
        ...
        public void AddResult(SmokeTestResult result)
        {
            ...
            _logger.Information("Smoke test result: {TestName} - {Success} - {Category} - {Duration}ms",
