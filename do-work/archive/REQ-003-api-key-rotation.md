...
Implemented a comprehensive API key rotation and management system.

Key features:
- **Lifecycle Management**: New `ApiKeyMetadata` and `ApiKeyVersion` models support multiple versions per provider, status tracking (Active, Deprecated, Expired, Revoked), and expiration dates.
- **Secure Storage**: Actual key values remain in Windows Credential Manager, while rich metadata is managed via `ApiKeyManagementService` and stored in a local JSON repository.
- **Zero-Downtime Rotation**: The `RotateKeyAsync` workflow creates a new version while keeping the old one active for a grace period, ensuring continuous service.
- **Automatic Monitoring**: `ApiKeyRotationService` runs as a background process every 12 hours to check for expiring keys and send proactive toast notifications to the user.
- **Usage Tracking**: Integrated with `WhisperService` to record success/failure rates and token usage per API key version.
- **Audit Integration**: All rotation, registration, and revocation events are logged through the SOC 2 compliant `IAuditLoggingService`.
...
**New tests verified:**
- Key registration and initial versioning
- Key rotation with version incrementing
- Active key retrieval from Credential Manager
- Usage statistics recording and persistence
