...
namespace WhisperKey.Infrastructure.SmokeTesting.Deployment
{
    /// <summary>
    ...
    public class DeploymentValidator : SmokeTestFramework
    {
        ...
        public override async Task<SmokeTestSuiteResult> RunAllTestsAsync()
        {
            ...
            try
            {
                ...
                _logger.LogInformation("Deployment validation completed: {PassedTests}/{TotalTests} passed", 
            ...
            {
                _logger.LogError(ex, "Deployment validation failed with exception");
        ...
        private async Task<SmokeTestResult> TestServiceAvailabilityAsync()
        {
            ...
            try
            {
                ...
                var services = new List<(string Name, Func<Task<bool>> Check)>
                {
                    ...
                    ("AudioDeviceService", async () => { await _audioDeviceService.GetInputDevicesAsync(); return true; }),
