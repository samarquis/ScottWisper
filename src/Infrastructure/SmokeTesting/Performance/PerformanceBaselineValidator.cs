...
namespace WhisperKey.Infrastructure.SmokeTesting.Performance
{
    /// <summary>
    ...
    public class PerformanceBaselineValidator : SmokeTestFramework
    {
        ...
        public override async Task<SmokeTestSuiteResult> RunAllTestsAsync()
        {
            ...
            try
            {
                ...
                _logger.LogInformation("Performance baseline validation completed: {PassedTests}/{TotalTests} passed", 
            ...
            {
                _logger.LogError(ex, "Performance baseline validation failed with exception");
        ...
        private async Task<SmokeTestResult> TestAudioProcessingPerformanceAsync()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    await _audioDeviceService.GetInputDevicesAsync();
                }
                multiStart.Stop();
        ...
        private async Task<SmokeTestResult> TestSettingsLoadPerformanceAsync()
        {
            ...
            try
            {
                ...
                await _settingsService.SaveSettingsAsync();
                settings.Transcription.EnableRateLimiting = originalTestMode; // Restore
                await _settingsService.SaveSettingsAsync();
                modifyStart.Stop();
