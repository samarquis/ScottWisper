...
namespace WhisperKey.Infrastructure.SmokeTesting.HealthChecks
{
    ...
    public class DatabaseHealthChecker : SmokeTestFramework
    {
        ...
        public override async Task<SmokeTestSuiteResult> RunAllTestsAsync()
        {
            ...
            try
            {
                _logger.LogInformation("Starting database health checks");
                ...
                _logger.LogInformation("Database health checks completed: {PassedTests}/{TotalTests} passed", 
            ...
            {
                _logger.LogError(ex, "Database health checks failed with exception");
        ...
        private async Task<SmokeTestResult> TestDatabaseOperationsAsync()
        {
            ...
            try
            {
                ...
                var originalValue = settings.Transcription.EnableRateLimiting;
                settings.Transcription.EnableRateLimiting = !originalValue; // Toggle value
                await _settingsService.SaveSettingsAsync();
                settings.Transcription.EnableRateLimiting = originalValue; // Restore original value
                await _settingsService.SaveSettingsAsync();
                writeStart.Stop();
        ...
        private async Task<SmokeTestResult> TestDataIntegrityAsync()
        {
            ...
            try
            {
                ...
                var originalApiKey = testSettings.Transcription.ApiKey;
                var originalHotkey = testSettings.Hotkeys.ToggleRecording;
                var originalTestMode = testSettings.Transcription.EnableRateLimiting;

                // Set test values
                testSettings.Transcription.ApiKey = "test-integrity-key-" + Guid.NewGuid().ToString();
                testSettings.Hotkeys.ToggleRecording = "Ctrl+Shift+I";
                testSettings.Transcription.EnableRateLimiting = true;
                ...
                {
                    ("ApiKeyMatch", loadedSettings.Transcription.ApiKey == testSettings.Transcription.ApiKey),
                    ("HotkeyMatch", loadedSettings.Hotkeys.ToggleRecording == testSettings.Hotkeys.ToggleRecording),
                    ("TestModeMatch", loadedSettings.Transcription.EnableRateLimiting == testSettings.Transcription.EnableRateLimiting)
                };
                ...
                testSettings.Transcription.ApiKey = originalApiKey;
                testSettings.Hotkeys.ToggleRecording = originalHotkey;
                testSettings.Transcription.EnableRateLimiting = originalTestMode;
                await _settingsService.SaveSettingsAsync();
        ...
        private async Task<SmokeTestResult> TestDatabasePerformanceAsync()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    settings.Transcription.EnableRateLimiting = !settings.Transcription.EnableRateLimiting;
                    await _settingsService.SaveSettingsAsync();
                }
                writeStart.Stop();
    ...
    public class AuthenticationHealthChecker : SmokeTestFramework
    {
        ...
        public override async Task<SmokeTestSuiteResult> RunAllTestsAsync()
        {
            ...
            try
            {
                _logger.LogInformation("Starting authentication health checks");
                ...
                _logger.LogInformation("Authentication health checks completed: {PassedTests}/{TotalTests} passed", 
                    suiteResult.PassedTests, suiteResult.TotalTests);
            ...
            {
                _logger.LogError(ex, "Authentication health checks failed with exception");
