...
namespace WhisperKey.Services
{
    ...
    public class ValidationService
    {
        ...
        public async Task<ValidationReport> RunPhase1Validation()
        {
            ...
            await ValidateGlobalHotkey().ConfigureAwait(false);

            // 2. Speech-to-text conversion
            await ValidateSpeechToText().ConfigureAwait(false);

            // 3. Real-time text output
            await ValidateRealTimeOutput().ConfigureAwait(false);

            // 4. Windows compatibility
            await ValidateWindowsCompatibility().ConfigureAwait(false);

            // 5. Free tier usage sustainability
            await ValidateFreeTierUsage().ConfigureAwait(false);

            // 6. Performance requirements
            await ValidatePerformanceRequirements().ConfigureAwait(false);

            return GenerateReport();
        }

        private Task ValidateGlobalHotkey()
        {
        ...
        private async Task ValidateSpeechToText()
        {
            ...
            try
            {
                ...
                var apiKey = await _credentialService.RetrieveCredentialAsync("OpenAI_ApiKey").ConfigureAwait(false);
        ...
        private Task ValidateRealTimeOutput()
        {
            var result = new ValidationResult
            {
        ...
        private Task ValidateWindowsCompatibility()
        {
            var result = new ValidationResult
            {
        ...
        private Task ValidateFreeTierUsage()
        {
            var result = new ValidationResult
            {
        ...
        private Task ValidatePerformanceRequirements()
        {
            var result = new ValidationResult
            {
